---
name: now
description: Daily workflow router. Determines whether to run /prep, /go, or /wrap-up based on current time, today's daily note state, and user intent. Use when the user says "now", "what should I do", "continue my day", "run my day", "I'm stuck", "help me get back on track", "what mode am I in", or gives ambiguous daily-workflow input.
argument-hint: "[optional: context about what's happening right now]"
disable-model-invocation: true
---

You are the front door for the daily workflow loop: `/prep` → `/go` → `/wrap-up` → `/prep`.
...
| User intent | Route |
|---|---|
| "prep", "plan today", "plan tomorrow", "set up my day" | `/prep` |
...
| Condition | Route |
|---|---|
| Note missing before 12:00 | `/prep` for today |
| Note missing after 12:00 and user asks what to do now | `/go` micro-prep |
| Note exists but has no Shaped Goals and no Rolling Timeblocks before 12:00 | `/prep` for today |
...
| Reflection already exists and user asks what's next | `/prep` for tomorrow |
...
- `/prep`: `"You're in prep. Let's set up the day."`
...
If routing to `/prep`, pass any context from `$ARGUMENTS`.
...
- Never do full planning inside `/now`; route to `/prep` or `/go` micro-prep.
...
- Prefer `/prep` when no day exists yet or tomorrow needs setup.
